...
namespace Infrastructure.Repository
{
    public class Account(UserManager<ApplicationUser> userManager, SignInManager<ApplicationUser> signInManager, AppDbContext context) : IAccount
    {
        public async Task<ServiceResponse> CreateUserAsync(CreateUserRequestDTO model)
        {
            ...
            if (user != null)
                return new ServiceResponse(false, "User exists already");
        ...
        {
            if (string.IsNullOrEmpty(model.Policy)) return new ServiceResponse(false, "No policy specified yet");
            Claim[] userClaims = [];
        ...
        public async Task<ServiceResponse> LoginAsync(LoginUserRequestDTO model)
        {
            ...
            if (!result.Succeeded) return new ServiceResponse(false, "Unknown error occured while logging in");
        ...
        public async Task SetUpAsync()
        {
            try
            {
                ...
                Console.WriteLine($"Setup Result: {response.Flag}, Message: {response.Message}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error in SetUpAsync: {ex.Message}");
            }
        }
